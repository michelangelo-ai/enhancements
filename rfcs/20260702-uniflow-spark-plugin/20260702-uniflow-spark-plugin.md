...
- [x] ~~`SparkTask` has a retry loop (`retry_attempts`) — should `run_spark_job()` support the same at the Python level, or is relying on `@workflow()`-level retry (if any) sufficient?~~ **Resolved:** yes — `run_spark_job()` takes a `retry_attempts: int = 0` param, backed by a matching retry loop in the `run_job` Go builtin. Both retry on **Failed and Killed identically**, since `Killed` is commonly caused by infra instability (node preemption, resource pressure, node restarts) rather than deliberate cancellation; deliberate cancellation is handled entirely separately by `doSensorJob`'s `workflow.IsCanceledError` branch, which tears down the Cadence context and calls `TerminateSparkJob` before the retry loop is ever reached.
