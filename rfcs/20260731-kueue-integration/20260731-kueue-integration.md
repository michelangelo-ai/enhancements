...
| 3 | Queue visibility | A queued cluster reports the non-terminal `RAY_CLUSTER_STATE_SUSPENDED` (PR #1700) rather than `UNKNOWN`; `kubectl get workload -A` / `kubectl describe clusterqueue` give queue depth and admission errors; scheduler reason strings (`kueue:*`) explain assignment decisions. | Partial — "queued" is now distinguishable from "broken"; reflecting Kueue's *reason* and a UI affordance is an Open question |
| 4 | Hardware-aware routing | `ResourceFlavor` is exactly this abstraction (per-SKU flavors + fallback order via operator YAML); Phase 2's headroom scoring additionally routes jobs *across clusters* by live capacity. | Mechanism designed; per-SKU defaults deferred (Non-goals / Open questions) |
